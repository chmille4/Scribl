...
var Glyph = Class.extend({
	...
	setTextOptions : function() {
		...
		if (glyph.font.style == undefined && glyph.track.chart.gene.text.style != undefined)
			glyph.font.style = glyph.track.chart.gene.text.style
		
		// set font size
		// check if gene level has not been set but chart level has
		if (glyph.font.size == undefined && glyph.track.chart.gene.text.size != undefined)
			glyph.font.size = glyph.track.chart.gene.text.size
		
		// set color
		// check if gene level has not been set but chart level has
		if (glyph.font.color == undefined && glyph.track.chart.gene.text.color != undefined)
			glyph.font.color = glyph.track.chart.gene.text.color
			
		// set text align
		// check if gene level has not been set but chart level has
		if (glyph.font.align == undefined && glyph.track.chart.gene.text.align != undefined)
			glyph.font.align = glyph.track.chart.gene.text.align
	},
	...
	getFillStyle : function() {
		...
		if (glyph.color != undefined)
			color = glyph.color
		else if ( glyph.track.chart.gene.color != undefined)
			color = glyph.track.chart.gene.color;
		else {
			var lineargradient2 = glyph.ctx.createLinearGradient(glyph.length/2,0,glyph.length/2, glyph.getHeight()); 
			for (var i = 0; i < glyph.track.chart.gene.linearGradient.length ; i++ ) {
				var colorPer = i / (glyph.track.chart.gene.linearGradient.length - 1);
				lineargradient2.addColorStop(colorPer, glyph.track.chart.gene.linearGradient[i]);
			}  
