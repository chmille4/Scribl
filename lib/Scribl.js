	...
	function Scribl(canvas, width) {
		...
		this.determineMajorTick = function() {
			...
			// var rs = this.scale.max - this.scale.min
			// var dived = rs/numtimes
			// var dms = dived / this.tick.major.size + .4
			// alert("tick =  " +  " region size = " + rs + " divedNumTimes = " + dived + " dms = " + dms );
		...
		this.handleMouseEvent = function(e, type) {
			...
			if (type == 'click') {
			    // jsa
			    var clickFns = chart.events.clicks;
			    for (var i = 0; i < clickFns.length; i++) {
				clicks[i](chart);
			    }
			    // for ( var i in this.events.clicks ) {
			    //chart.events.clicks[i](chart);
			    //}
			} else {
			    // jsa
			    ...
			    //chart.events.mouseovers[i](chart);
			    //}
			}
	...
	function track(ctx) {
		...
		this.addGene = function( position, length, strand) {
			...
			// create gene
			var gene_new = new Gene(ctx, position, length, strand); 
			gene_new.track = this;
