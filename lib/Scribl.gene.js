	...
	function Gene(ctx, position, length, height, strand) {
		...
		this.getColor = function() {
			...
			// check if default color was ovewridden on a chart level
			if ( this.track.chart.gene.linearGradient != undefined) {
				...
				lineargradient2.addColorStop(1, this.track.chart.gene.linearGradient[1]);
		...
		this.getRoundness = function() { 
			...
			// check if roundness was overridden on a chart level
			if ( this.track.chart.gene.roundness != undefined )
				roundness = this.track.chart.gene.roundness;
			else
				roundness = this.roundness;
				
			return (this.height * roundness/100); 
		}
		...
		this.draw = function(percentScale) {
			...
			drawGene(this, this.name, this.position*percentScale, this.length*percentScale, this.height, this.getRoundness(), this.slope, this.strand, this.getColor());	
