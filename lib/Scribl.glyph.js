...
var Glyph = Class.extend({
	...
	setTextOptions : function() {
		...
		var glyph = this;
		var chartLevelGlyph = "glyph.track.chart." + glyph.type;
		
		// set style
		// check if individual level has not been set but glyph level has
		if (glyph.font.style == undefined && glyph.track.chart.glyph.text.font != undefined)
			glyph.font.style = glyph.track.chart.glyph.text.font
		else if ( eval(chartLevelGlyph).text.style != undefined ) 
			glyph.font.style = eval(chartLevelGlyph).text.font
		
		// set font size
		// check if individual level has not been set but glyph level has
		if (glyph.font.size == undefined && glyph.track.chart.glyph.text.size != undefined)
			glyph.font.size = glyph.track.chart.glyph.text.size
		else if ( eval(chartLevelGlyph).text.size != undefined )
			glyph.font.size = eval(chartLevelGlyph).text.size
		
		// set color
		// check if individual level has not been set but glyph level has
		if (glyph.font.color == undefined && glyph.track.chart.glyph.text.color != undefined)
			glyph.font.color = glyph.track.chart.glyph.text.color
		else if ( eval(chartLevelGlyph).text.color != undefined ) 
			glyph.font.color = eval(chartLevelGlyph).text.color
			
		// set text align
		// check if individual level has not been set but glyph level has
		if (glyph.font.align == undefined && glyph.track.chart.glyph.text.align != undefined)
			glyph.font.align = glyph.track.chart.glyph.text.align
		else if ( eval(chartLevelGlyph).text.align != undefined ) 
