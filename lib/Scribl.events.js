function MouseEventHandler(chart) {
	...
	this.handleMouseover = function() {
		var eventDisplay = undefined;
		var obj = this.eventElement;
		var ctx = this.chart.ctx;

		// if (obj == undefined || obj.onMouseover == undefined)
		// 			eventDisplay = "Nothing Selected";
		// 		else 
		// 			eventDisplay = obj.onMouseover;
		// 
		// 		ctx.fillStyle="black";
		// 		ctx.fillText(eventDisplay, 10, 25);
		
		if (obj != undefined && obj.onMouseover != undefined)
			this.tooltip.fire(obj);
	}
...
function tooltips(chart) {
	...
	this.fire = function(obj, style) {  		
		...
		// reset opacity to value before tooltp.fireß
...
//		ctx.save
//		ctx.translate(x,y);
//		drawText(ctx, obj.onMouseover, length, height);
//		ctx.restore();
	}
}