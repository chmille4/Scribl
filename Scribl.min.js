...
this.tick.halfColor="rgb(10,10,10)";this.tooltips={};this.tooltips.text={};this.tooltips.text.font="arial";this.tooltips.text.size=12;this.tooltips.borderWidth=1;this.tooltips.roundness=5;this.tooltips.fade=!1;this.tooltips.style="light";this.scrollable=!1;this.scrollValues=[0,void 0];this.drawStyle="expand";this.myMouseEventHandler=new MouseEventHandler(this);this.tracks=[]},getScaleHeight:function(){return this.scale.font.size+this.scale.size},getHeight:function(){var a=0;this.scale.off||(a+=this.getScaleHeight());
for(var b=this.tracks.length,c=0;c<b;c++)a+=this.laneBuffer,a+=this.tracks[c].getHeight();return a},addTrack:function(){var a=new Track(this.ctx);a.chart=this;if(this.tracks.length==1&&this.tracks[0]==void 0)this.tracks=[];this.tracks.push(a);return a},loadGenbank:function(a){genbank(a,this)},loadBed:function(a){bed(a,this)},loadFeatures:function(a){for(var b=0;b<a.length;b++)this.addFeature(a[b])},addGene:function(a,b,c,d){return this.addFeature(new BlockArrow("gene",a,b,c,d))},addProtein:function(a,
b,c,d){return this.addFeature(new BlockArrow("protein",a,b,c,d))},addFeature:function(a){(this.tracks[0]||this.addTrack()).addFeature(a);return a},slice:function(a,b,c){c=c||"inclusive";for(var d=this.tracks.length,e=new Scribl(this.canvas,this.width),g=0;g<d;g++){var i=this.tracks[g],k=e.addTrack();k.drawStyle=i.drawStyle;for(var j=i.lanes.length,m=0;m<j;m++){newLane=k.addLane();for(var l=i.lanes[m].features,n=0;n<l.length;n++){var o=l[n].position+l[n].length,p=l[n].position;if(c=="inclusive")p>=
a&&p<=b?newLane.addFeature(l[n]):o>a&&o<b?newLane.addFeature(l[n]):p<a&&o>b?newLane.addFeature(l[n]):p>a&&o<b&&newLane.addFeature(l[n]);else if(c=="strict")if(p>=a&&p<=b)if(o>a&&o<b)newLane.addFeature(l[n]);else{var q=l[n].clone();q.length=Math.abs(b-p);newLane.addFeature(q)}else if(o>a&&o<b)q=l[n].clone(),q.position=a,q.length=Math.abs(o-a),newLane.addFeature(q);else{if(p<a&&o>b)q=l[n].clone(),q.position=a,q.length=Math.abs(b-a),newLane.addFeature(q)}else c=="exclusive"&&p>=a&&p<=b&&o>a&&o<b&&newLane.addFeature(l[n])}}}e.laneSizes=
...
a.measureText(this.getTickText(this.scale.min)).width/2+10:a.measureText("0").width/2+10;a.save();if(!this.scale.off){var c=a.fillStyle;this.drawScale();a.fillStyle=c}a.save();this.scale.off||a.translate(0,this.getScaleHeight()+this.laneBuffer);for(c=0;c<b.length;c++)b[c].draw();a.restore();a.restore();a.restore();this.events.added||this.registerEventListeners()},redraw:function(){this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height);this.tracks.length>0&&this.draw()},drawScale:function(){var a=
this.ctx,b=this.scale.font.size+this.scale.size,c=this.scale.font.size+2,d=this.scale.font.size+this.scale.size*0.66,e=this.scale.font.size+this.scale.size*0.33;a.font=this.scale.font.size+"px arial";a.textBaseline="top";a.fillStyle=this.scale.font.color;for(var g=this.scale.min%this.tick.minor.size==0?this.scale.min:this.scale.min-this.scale.min%this.tick.minor.size+this.tick.minor.size;g<=this.scale.max;g+=this.tick.minor.size){a.beginPath();var i=this.pixelsToNts(g-this.scale.min)+this.offset;
if(g%this.tick.major.size==0){var k=this.getTickText(g);a.textAlign="center";a.fillText(k,i,0);a.moveTo(i,b);a.lineTo(i,c);a.strokeStyle=this.tick.major.color}else a.moveTo(i,b),g%(this.tick.major.size/2)==0?(a.strokeStyle=this.tick.halfColor,a.lineTo(i,e)):(a.strokeStyle=this.tick.minor.color,a.lineTo(i,d));a.stroke()}},pixelsToNts:function(a){return a==void 0?this.width/(this.scale.max-this.scale.min):this.width/(this.scale.max-this.scale.min)*a},ntsToPixels:function(a){return a==void 0?1/this.pixelsToNts():
...
1];if(e!=void 0&&a.position-3/this.chart.pixelsToNts()>e.position+e.length){c=!1;b=this.lanes[d];break}}c&&(b=this.addLane());b.addFeature(a);return a},getDrawStyle:function(){return this.drawStyle?this.drawStyle:this.chart.drawStyle},getHeight:function(){var a=0,b=this.lanes.length,c=this.chart.laneBuffer,d=this.getDrawStyle();if(d=="line"||d=="collapse")b=1;for(d=0;d<b;d++)a+=c,a+=this.lanes[d].getHeight();return a},getPixelPositionY:function(){for(var a=this.chart.getScaleHeight(),b=this.getHeight(),
c=0;c<this.chart.tracks.length;c++){a+=this.chart.laneBuffer;if(this.uid==this.chart.tracks[c].uid)break;a+=b}return a},calcCoverageData:function(){for(var a=this.lanes,b=0;b<a.length;b++)for(var c=0;c<a[b].features.length;c++){var d=a[b].features[c],e=Math.round(d.getPixelPositionX());for(d=Math.round(e+d.getPixelLength());e<=d;e++)this.coverageData[e]=this.coverageData[e]+1||1,this.maxDepth=Math.max(this.coverageData[e],this.maxDepth)}},draw:function(){var a=this.getDrawStyle(),b=this.chart.laneSizes,
c=this.lanes,d=this.chart.laneBuffer,e=b+d,g=this.ctx;if(a==void 0||a=="expand"){g.translate(0,d);for(a=0;a<c.length;a++)c[a].y=e,c[a].draw(),b=c[a].getHeight(),g.translate(0,b+d),e=e+b+d}else if(a=="collapse"){b=[];for(a=0;a<c.length;a++)b=b.concat(c[a].features);b.sort(function(a,b){return a.position-b.position});for(e=0;e<b.length;e++){a=b[e].length;var i=b[e].name,k=void 0;for(k=e+1;k<b.length;k++)if(b[e].getEnd()>=b[k].position)b[e].length=Math.max(b[e].getEnd(),b[k].getEnd())-b[e].position,
b[e].name="";else break;b[e].draw();b[e].length=a;b[e].name=i;e=k-1}g.translate(0,c[0].getHeight()+d)}else if(a=="line"){this.coverageData.length==0&&this.calcCoverageData();e=this.maxDepth;g.beginPath();for(a=this.chart.offset;a<=this.chart.width+this.chart.offset;a++)i=this.coverageData[a]/e*b||0,i=b-i,g.lineTo(a,i);g.lineTo(this.chart.width+this.chart.offset,b);g.stroke();g.translate(0,c[0].getHeight()+d)}}});var Lane=Class.extend({init:function(a,b){this.height=void 0;this.features=[];this.ctx=a;this.track=b;this.chart=b.chart;this.uid=_uniqueId("lane")},addGene:function(a,b,c,d){return this.addFeature(new BlockArrow("gene",a,b,c,d))},addProtein:function(a,b,c,d){return this.addFeature(new BlockArrow("protein",a,b,c,d))},addFeature:function(a){a.lane=this;this.features.push(a);this.chart[a.type]||(this.chart[a.type]={text:{}});if(a.length+a.position>this.chart.scale.max||this.chart.scale.max==void 0)this.chart.scale.max=
a.length+a.position;if(a.position<this.chart.scale.min||this.chart.scale.min==void 0)this.chart.scale.min=a.position;return a},loadFeatures:function(a){for(var b=a.length,c=0;c<b;c++)this.addFeature(a[c])},getHeight:function(){return this.height!=void 0?this.height:this.chart.laneSizes},getPixelPositionY:function(){for(var a=this.track.getPixelPositionY(),b=this.getHeight(),c=0;c<this.track.lanes.length;c++){a+=this.track.chart.laneBuffer;if(this.uid==this.track.lanes[c].uid)break;a+=b}return a},
...
c.push(">")}else c.push("/>")}return c.join("")};var Glyph=Class.extend({init:function(a,b,c,d,e){this.uid=_uniqueId("feature");this.position=b;this.length=c;this.strand=d;this.type=a;this.name="";this.borderColor="none";this.borderWidth=void 0;this.text={};this.text.font=void 0;this.text.size=void 0;this.text.color=void 0;this.onClick=this.text.align=void 0;for(var g in e)this[g]=e[g]},setColorGradient:function(){if(arguments.length==1)this.color=arguments[0];else{for(var a=this.lane.ctx.createLinearGradient(this.length/2,0,this.length/2,this.getHeight()),
b,c=0;b=arguments[c],c<arguments.length;c++)a.addColorStop(c/(arguments.length-1),b);this.color=a}},getPixelLength:function(){return this.lane.chart.pixelsToNts(this.length)||1},getPixelPositionX:function(){var a=parseInt(this.lane.track.chart.offset)||0;return this.lane.chart.pixelsToNts(this.position-this.lane.track.chart.scale.min)+a},getPixelPositionY:function(){return this.lane.getPixelPositionY()},getEnd:function(){return this.position+this.length},clone:function(){var a=this;return a=f.orientation?
eval("new"+f.glyphType+"("+f.type+","+f.position+","+f.length+","+f.orientation):eval("new"+f.glyphType+"("+f.type+","+f.position+","+f.length)},getAttr:function(a){a=a.split("-");for(var b=this,c=0;c<a.length;c++)b=b[a[c]];if(b)return b;if(this.parent){b=this.parent;for(c=0;c<a.length;c++)b=b[a[c]];if(b)return b}b=this.lane.chart[this.type];for(c=0;c<a.length;c++)b=b[a[c]];if(b)return b;b=this.lane.chart.glyph;for(c=0;c<a.length;c++)b=b[a[c]];if(b)return b},drawText:function(a){var b=this.lane.chart.ctx,
c=this.getPixelLength(),d=this.getHeight(),e=this.getAttr("text-size"),g=this.getAttr("text-style");b.font=e+"px "+g;b.textBaseline="middle";b.fillStyle=this.getAttr("text-color");var i=void 0,k=this.getAttr("text-align");k=="start"?k=this.strand=="+"?"left":"right":k=="end"&&(k=this.strand=="+"?"right":"left");b.textAlign=k;k=="left"?i=5:k=="center"?i=c/2:k=="right"&&(i=c-5);var j=b.measureText(a);if(a&&a!=""){for(;c-j.width<4;)if(e=/^\d+/.exec(b.font),e--,j=b.measureText(a),b.font=e+"px "+g,e<=
8){a="";break}this.glyphType=="Complex"&&(c=0,e=/^\d+/.exec(b.font),k=="center"&&(c=-(b.measureText(a).width/2+2.5)),b.clearRect(i+c,d/2-e/2,b.measureText(a).width+5,e));b.fillText(a,i,d/2)}},calcRoundness:function(){return this.getHeight()*this.getRoundness()/100},isContainedWithinRect:function(a,b,c,d){var e=this.getPixelPositionY(),g=this.getPixelPositionX(),i=this.getPixelPositionX()+this.getPixelLength(),k=e+this.getHeight();return g>=a&&i<=c&&e>=b&&k<=d},getRoundness:function(){var a=this.lane.chart[this.type];
return this.roundness!=void 0?this.roundness:this.parent&&this.parent.roundness!=void 0?this.parent.roundness:a!=void 0&&a.roundness!=void 0?a.roundness:this.lane.chart.glyph.roundness},getHeight:function(){return this.lane.getHeight()},getStrokeStyle:function(){var a;a=this.lane.chart[this.type];if(this.borderColor!=void 0)a=this.borderColor;else if(this.parent&&this.parent.borderColor!=void 0)a=this.parent.borderColor;else if(a.borderColor!=void 0)a=a.borderColor;else if(a.linearGradient!=void 0){for(var b=
this.ctx.createLinearGradient(this.length/2,0,this.length/2,this.getHeight()),c=0;c<a.linearGradient.length;c++)b.addColorStop(c/(a.linearGradient.length-1),a.linearGradient[c]);a=b}else a=this.lane.chart.glyph.borderColor!=void 0?this.lane.chart.glyph.borderColor:black;return a},getLineWidth:function(){var a=this.lane.chart[this.type];return this.borderWidth!=void 0?this.borderWidth:this.parent&&this.parent.borderWidth!=void 0?this.parent.borderWidth:a.borderWidth!=void 0?a.borderWidth:this.lane.chart.glyph.borderWidth!=
void 0?this.lane.chart.glyph.borderWidth:1},getFillStyle:function(){var a=this.getAttr("color");if(typeof a=="object"){for(var b=this.lane.ctx.createLinearGradient(this.length/2,0,this.length/2,this.getHeight()),c,d=0;c=a[d],d<a.length;d++)b.addColorStop(d/(a.length-1),c);return b}else return a},isSubFeature:function(){return this.parent!=void 0},clearInside:function(){this.ctx.clearRect(0,0,this.getPixelLength(),this.getHeight())},draw:function(){this.ctx=this.lane.chart.ctx;this.ctx.beginPath();
/^\d+/.exec(this.ctx.font);var a=/\S+$/.exec(this.ctx.font);this.onClick=this.getAttr("onClick");this.ctx.fillStyle=this.getFillStyle();var b=this.ctx.fillStyle,c=this.getPixelPositionX(),d=this.getHeight();d<10?this.ctx.font="10px "+a:this.ctx.font=d*0.9+"px "+a;this.ctx.translate(c,0);this.strand=="-"&&!this.isSubFeature()&&this.ctx.transform(-1,0,0,1,this.getPixelLength(),0);this._draw();if(this.borderColor!="none")this.color=="none"&&this.parent.glyphType=="Complex"&&this.clearInside(),a=this.ctx.strokeStyle,
d=this.ctx.lineWidth,this.ctx.strokeStyle=this.getStrokeStyle(),this.ctx.lineWidth=this.getLineWidth(),this.ctx.stroke(),this.ctx.strokeStyle=a,this.ctx.lineWidth=d;this.color!="none"&&this.ctx.fill();this.strand=="-"&&!this.isSubFeature()&&this.ctx.transform(-1,0,0,1,this.getPixelLength(),0);this.drawText(this.name);this.ctx.translate(-c,0);this.ctx.fillStyle=b;this.lane.chart.myMouseEventHandler.addEvents(this)}});var BlockArrow=Glyph.extend({init:function(a,b,c,d,e){this._super(a,b,c,d,e);this.slope=1;this.glyphType="BlockArrow"},_draw:function(a,b,c,d){a=a||this.ctx;b=b||this.getPixelLength();c=c||this.getHeight();d=d+1||this.calcRoundness();d!=void 0&&(d-=1);tc_ctrl_y=tc_ctrl_x=x=y=0;tc_lgth_x=x+d;tc_lgth_y=y;tc_wdth_x=x;tc_wdth_y=y+d;bc_ctrl_x=x;bc_ctrl_y=y+c;bc_lgth_x=x+d;bc_lgth_y=y+c;bc_wdth_x=x;bc_wdth_y=y+c-d;a_b_x=x+b-d;a_t_x=x+b-d;a_max_x=x+b;t=0.5;a_ctrl_x=(a_max_x-(1-t)*(1-t)*a_b_x-t*t*a_t_x)/
