...
this.tick.major.size=10;this.tick.major.color="black";this.tick.minor={};this.tick.minor.size=1;this.tick.minor.color="rgb(55,55,55)";this.tick.halfColor="rgb(10,10,10)";this.tooltips={};this.tooltips.text={};this.tooltips.text.font="arial";this.tooltips.text.size=12;this.tooltips.borderWidth=1;this.tooltips.roundness=5;this.tooltips.fade=!1;this.tooltips.style="light";this.scrollable=!1;this.scrollValues=[0,void 0];this.drawStyle="expand";this.myMouseEventHandler=new MouseEventHandler(this);this.tracks=
[]},getScaleHeight:function(){return this.scale.font.size+this.scale.size},getHeight:function(){var a=0;this.scale.off||(a+=this.getScaleHeight());for(var b=this.tracks.length,c=0;c<b;c++)a+=this.trackBuffer,a+=this.tracks[c].getHeight();return a},addScale:function(){this.scale.positions.push(this.tracks.length)},addTrack:function(){var a=new Track(this.ctx);a.chart=this;if(this.tracks.length==1&&this.tracks[0]==void 0)this.tracks=[];this.tracks.push(a);return a},loadGenbank:function(a){genbank(a,
this)},loadBed:function(a){bed(a,this)},loadFeatures:function(a){for(var b=0;b<a.length;b++)this.addFeature(a[b])},addGene:function(a,b,c,d){return this.addFeature(new BlockArrow("gene",a,b,c,d))},addProtein:function(a,b,c,d){return this.addFeature(new BlockArrow("protein",a,b,c,d))},addFeature:function(a){(this.tracks[0]||this.addTrack()).addFeature(a);return a},slice:function(a,b,c){c=c||"inclusive";for(var d=this.tracks.length,e=new Scribl(this.canvas,this.width),g=0;g<d;g++){var i=this.tracks[g],
k=e.addTrack();k.drawStyle=i.drawStyle;for(var j=i.lanes.length,m=0;m<j;m++){newLane=k.addLane();for(var l=i.lanes[m].features,n=0;n<l.length;n++){var o=l[n].position+l[n].length,p=l[n].position;if(c=="inclusive")p>=a&&p<=b?newLane.addFeature(l[n]):o>a&&o<b?newLane.addFeature(l[n]):p<a&&o>b?newLane.addFeature(l[n]):p>a&&o<b&&newLane.addFeature(l[n]);else if(c=="strict")if(p>=a&&p<=b)if(o>a&&o<b)newLane.addFeature(l[n]);else{var q=l[n].clone();q.length=Math.abs(b-p);newLane.addFeature(q)}else if(o>
a&&o<b)q=l[n].clone(),q.position=a,q.length=Math.abs(o-a),newLane.addFeature(q);else{if(p<a&&o>b)q=l[n].clone(),q.position=a,q.length=Math.abs(b-a),newLane.addFeature(q)}else c=="exclusive"&&p>=a&&p<=b&&o>a&&o<b&&newLane.addFeature(l[n])}}}e.laneSizes=this.laneSizes;e.drawStyle=this.drawStyle;e.loadFeatures([]);return e},draw:function(){var a=this.ctx,b=this.tracks;this.scrollable==!0&&this.initScrollable();a.save();if(this.scale.pretty){if(this.tick.auto)this.tick.major.size=this.determineMajorTick(),
this.tick.minor.size=Math.round(this.tick.major.size/10);if(this.scale.auto)this.scale.min-=this.scale.min%this.tick.major.size,this.scale.max=Math.round(this.scale.max/this.tick.major.size+0.4)*this.tick.major.size}this.offset=this.scale.min.offset?a.measureText(this.getTickText(this.scale.min)).width/2+10:a.measureText("0").width/2+10;a.save();for(var c=0;c<b.length;c++)!this.scale.off&&this.scale.positions.indexOf(c)!=-1&&this.drawScale(),b[c].draw();!this.scale.off&&this.scale.positions.indexOf(b.length)!=
-1&&this.drawScale();a.restore();a.restore();this.events.added||this.registerEventListeners()},redraw:function(){this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height);this.tracks.length>0&&this.draw()},drawScale:function(){var a=this.ctx,b=a.fillStyle,c=this.scale.font.size+this.scale.size,d=this.scale.font.size+2,e=this.scale.font.size+this.scale.size*0.66,g=this.scale.font.size+this.scale.size*0.33;a.font=this.scale.font.size+"px arial";a.textBaseline="top";a.fillStyle=this.scale.font.color;
for(var i=this.scale.min%this.tick.minor.size==0?this.scale.min:this.scale.min-this.scale.min%this.tick.minor.size+this.tick.minor.size;i<=this.scale.max;i+=this.tick.minor.size){a.beginPath();var k=this.pixelsToNts(i-this.scale.min)+this.offset;if(i%this.tick.major.size==0){var j=this.getTickText(i);a.textAlign="center";a.fillText(j,k,0);a.moveTo(k,c);a.lineTo(k,d);a.strokeStyle=this.tick.major.color}else a.moveTo(k,c),i%(this.tick.major.size/2)==0?(a.strokeStyle=this.tick.halfColor,a.lineTo(k,g)):
(a.strokeStyle=this.tick.minor.color,a.lineTo(k,e));a.stroke()}a.fillStyle=b;a.translate(0,this.getScaleHeight()+this.laneBuffer)},pixelsToNts:function(a){return a==void 0?this.width/(this.scale.max-this.scale.min):this.width/(this.scale.max-this.scale.min)*a},ntsToPixels:function(a){return a==void 0?1/this.pixelsToNts():a/this.width},initScrollable:function(){var a;if(!this.scrolled){a=document.createElement("div");var b=document.createElement("div"),c=document.createElement("div");c.id="scribl-zoom-slider";
c.className="slider";c.style.cssFloat="left";c.style.height=new String(this.canvas.height*0.5)+"px";c.style.margin="30px auto auto -20px";a.style.cssText=this.canvas.style.cssText;this.canvas.style.cssText="";parentWidth=parseInt(this.canvas.width)+25;a.style.width=parentWidth+"px";b.style.width=this.canvas.width+"px";b.style.overflow="auto";b.id="scroll-wrapper";this.canvas.parentNode.replaceChild(a,this.canvas);a.appendChild(c);b.appendChild(this.canvas);a.appendChild(b);$(b).dragscrollable({dragSelector:"canvas:first",
acceptPropagatedEvent:!1})}b=this.scale.max-this.scale.min;var d=this.scrollValues[1]||this.scale.max-b*0.35;a=this.scrollValues[0]!=void 0?this.scrollValues[0]:this.scale.max+b*0.35;var e=(d-a)/document.getElementById("scroll-wrapper").style.width.split("px")[0];e=b/e;this.canvas.width=e;this.width=e-30;schart=this;d=(d-a)/(this.scale.max-this.scale.min)*100;$(c).slider({orientation:"vertical",range:"min",min:1,max:100,value:d,slide:function(a,b){var c=schart.scale.max-schart.scale.min,d=b.value/
...
getTickTextDecimalPlaces:function(a){if(!this.tick.auto)return a;var b=a;a>=1E6?b=Math.round(b/(1E6/Math.pow(10,5)))+"m":a>=1E3&&(b=Math.round(b/(1E3/Math.pow(10,2)))+"k");return b},handleMouseEvent:function(a,b){this.myMouseEventHandler.setMousePosition(a);for(var c=this.myMouseEventHandler.mouseY,d,e=0;e<this.tracks.length;e++)for(var g=0;g<this.tracks[e].lanes.length;g++){var i=this.tracks[e].lanes[g].getPixelPositionY(),k=i+this.tracks[e].lanes[g].getHeight();if(c>=i&&c<=k){d=this.tracks[e].lanes[g];
break}}if(d){this.ctx.save();d.erase();this.ctx.translate(0,d.getPixelPositionY());d.draw();(e=this.LastToolTip)&&this.ctx.putImageData(e.pixels,e.x,e.y);this.ctx.restore();c=b=="click"?this.events.clicks:this.events.mouseovers;for(e=0;e<c.length;e++)c[e](this);this.myMouseEventHandler.reset(this)}},addClickEventListener:function(a){this.events.clicks.push(a)},addMouseoverEventListener:function(a){this.events.mouseovers.push(a)},registerEventListeners:function(){var a=this;this.events.mouseovers.length>
