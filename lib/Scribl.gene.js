	...
	function Gene(ctx, position, length, strand) {
		...
		this.draw = function(percentScale) {
			...
			drawGene(this, this.name, this.position*percentScale, this.length*percentScale, this.getHeight(), this.getRoundness(), this.slope, this.strand, this.getBackgroundColor());	
...
//			alert(ctx.textAlign);
			ctx.fillText(name, placement, height/2);
		}
	}
